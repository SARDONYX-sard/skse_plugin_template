...
/// FIXME: Define dll name(NOTE: same as name in dll_main/Cargo.toml)
const DLL_NAME: &str = "rust_skse_sample";
...
impl DestDir {
    fn to_path_buf(&self) -> PathBuf {
        match self {
            Self::Root | Self::Mo2 => {
                Path::new(&format!("./build/mods/{DLL_NAME}/SKSE/Plugins/")).to_path_buf()
            } // Self::Mo2 => Path::new(&format!("D:/GAME/ModOrganizer Skyrim SE/mods/{DLL_NAME}/SKSE/Plugins/"))
              //     .to_path_buf(),
        }
    }
}
...
fn build_main_dll(args: BuildArgs) -> Result<()> {
    ...
    let dest_dir = args.dest.unwrap_or(args.dest_mode.to_path_buf());
